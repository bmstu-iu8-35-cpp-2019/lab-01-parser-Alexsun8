...
Json::Json(const std::string &s) {
    size_t i = 0;
    while (s[i] == ' ' || s[i] == '\n')i++;
    if (s[i] != '{' && s[i] != '[') { throw std::logic_error("Начало другое, нет {"); }
    else {
        if (s[i] == '{') {
            i++;
            for (; i < s.size(); i++) {
                //  if (s[i] != ' ') {
                while (s[i] == ' ' || s[i] == '\n') i++;
                if (s[i] != '"' && s[i] != '\\') throw std::logic_error("Нет кавычек в начале строки");
                else {
                    if (s[i] == '\\')i++; //key
                    i++;
                    std::string key = "";
                    ...
                    if (s[i] != '"' && s[i] != '\\') { throw std::logic_error("нет кавычки, искл 3"); }
                    else {
                        if (s[i] == '\\')i++;
                        i++;
                        std::any val;
                        while (s[i] == ' ' || s[i] == '\n') i++;
                        if (s[i] != ':') { throw std::logic_error("искл 4 нет двоеточия "); }
                        else {
                            i++;
                            while (s[i] == ' ' || s[i] == '\n') i++;
                            if (s[i] == '{') {
                            ...
                            /*  std::vector<std::any> vector;
                              ...
                              while (s[i] != ']') {
                                      ...
                                      while ((s[i] >= '0' && s[i] <= '9') || s[i] == '.') {
                                          if (s[i] == '.') {
                                              if (point) {
                                                  point = false;
                                                  t += s[i];
                                              } else { throw std::logic_error("искл 5, в числах лишняя точка "); }
                                          } else {
                                              t += s[i];
                                          }
                                          i++;
                                  ...
                                  while(s[i] == ' '|| s[i] == '\n') i ++;
                                  if (s[i] != ',' && s[i] != ']')
                                      throw std::logic_error("искл 6 замятая или скобочка массива  ");
                              }
                                ...
                                while ((s[i] >= '0' && s[i] <= '9') || s[i] == '.') {
                                    if (s[i] == '.') {
                                        if (point) {
                                            point = false;
                                            t += s[i];
                                        } else { throw std::logic_error("искл 7 лишняя точка "); }
                                    } else {
                                        t += s[i];
                                    }
                                    i++;
                                }
                                if (point) { val = atoi(t.c_str()); }
                                else {
                                    val = atof(t.c_str());
                                }
                            }
                    ...
                    while ((s[i] >= '0' && s[i] <= '9') || s[i] == '.') {
                        if (s[i] == '.') {
                            if (point) {
                                point = false;
                                t += s[i];
                            } else { throw std::logic_error("искл 5, в числах лишняя точк 22 а"); }
                        } else {
                            t += s[i];
                        }
                        i++;
